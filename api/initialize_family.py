...
class Initalize_Family():
    ...
    def handle_family_list(self,res):
        ...
        # #处理掉加入的房间
        # joinfamilys=res_dict['data']['joinfamilies']
        # lenjoinfamilys=len(joinfamilys)
        # for i in range(lenjoinfamilys):
        #     tmp_family_list = []
        #     # print(i)
        #     tmp_family = lenjoinfamilys[i]
        #     family_name = tmp_family['familyName']
        #     family_id = tmp_family['familyId']
        #     tmp_family_list.append(family_name)
        #     tmp_family_list.append(family_id)
        #     join_family_list.append(tmp_family_list)
    ...
    def exit_family(self,family_join):
        # 退出家庭
        url = "/emuplus/family/v1/member/exit"
        family_name = family_join[0]
        familyId = family_join[1]
        logging.info("要退出的房间是：" + str(family_name))
        logging.info("要退出的房间id是： " + str(familyId))
        ...
        try:
            ...
            print(e)
            logging.error("退出房间发送请求错误，错误类型是：")
            logging.error(e.__class__.__name__)
            logging.error("发送请求错误，错误明细：")
            logging.error(e)
            real_txt = '请求异常'
            ...
            if res.status_code == 200 and "00000" in res.text:
                logging.info(family_name + "退出房间成功")
                return True, res
            else:
                return False, ''
    ...
    def exit_join_family(self,join_famliy_list):
        logging.info("开始准备退出房间")
        res = []
        print("开始准备退出房间")
        if join_famliy_list == None or len(join_famliy_list) == 0 or join_famliy_list == "":
            logging.warning("没有要退出的加入的房间，结束")
            # logging.info("没有要删除的房间，结束")
            return True, 'no_family_join'
        else:
            for family in join_famliy_list:
                logging.info("账号下的加入账号列表： ")
                logging.info(join_famliy_list)
                logging.info("当前处理的家庭： ")
                logging.info(family)
                print(family[0])
                if "NewFamily" in family[0]:
                    logging.info("退出"+str(family[0]))
                    #flag,res_tmp=self.delete_family(family)
                    flag,res_tmp=self.exit_join_family(family)

                    if flag:
                        res.append(res_tmp.text)

                else:
                    logging.info("没有找到加入的NewFamily房间")
                    res.append('no_delete_family_join')
        ...
        if flaglogin:
            ...
            if flag_family:
                ...
                try:
                    ...
                    #flag_join,res_tmp_join=self.exit_join_family(join_family_list)
