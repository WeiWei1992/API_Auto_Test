    ...
    def run(self):
        ...
        if "登录" == self.step:
            ...
            try:
                ...
                logging.error("登录接口发送请求异常，进入重试模式")
                headers = self.get_header(time_stamp, self.verify)
                logging.info("请求的heders: ")
                logging.info(headers)
                logging.info("body转码,防止有中午输入")
                self.body = self.body.encode("utf-8").decode("latin1")
                # 这个地方要加入重试机制
                for i in range(10):
                    try:
                        ...
                        logging.error("第 " + str(i) + "重试失败，等待后，再次重试")
                        wait_time = (i + 1) * 10
                        time.sleep(wait_time)
                        continue
                    else:
                        logging.warning("第"+str(i)+"次重试成功了，开始校验")
